...
public class GameController {
  ...
  public GameController(int count, int boardnum ,CPU b_cpu, CPU w_cpu, String bcpu, String wcpu) {
    cpus = new CPU[2];
    cpus[0] = b_cpu;
    cpus[1] = w_cpu;
    bcpu_name = bcpu;
    wcpu_name = wcpu;
    normalGame(count, boardnum);
  }

  //通常のゲーム
  public void normalGame(int count, int boardnum) {
  ...
  public void game(int boardnum) {
    //Game status
    GameState state = new GameState();
    state.reset(boardnum, state);
    cpus[0].reset();
    cpus[1].reset();
